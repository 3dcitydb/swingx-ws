...
public class JXMapKit extends JXPanel {
    ...
    public void setZoom(int zoom) {
    	int maxZoom = getMainMap().getTileFactory().getInfo().getMaximumZoomLevel();
        zoomChanging = true;
        mainMap.setZoom(zoom);
        miniMap.setZoom(mainMap.getZoom()+4 > maxZoom ? maxZoom : mainMap.getZoom()+4);
        if(sliderReversed) {
    ...
    private void initComponents() {
        ...
        miniMap.setBorder(javax.swing.BorderFactory.createLineBorder(new Color(0, 0, 0)));
    ...
    private WaypointPainter addressLocationPainter = new WaypointPainter() {
        public List<Waypoint> getWaypoints() {
            ...
            if(getAddressLocation() != null) {
                list.add(new Waypoint(getAddressLocation()));
            } else {
